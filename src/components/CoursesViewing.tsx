...
function CoursesViewing({ courses, onSelect, selectedId }: Props) {
  return (
    <section className="courses-section courses-viewing bg-white py-[70px]">
      <div className="mx-auto w-full max-w-[1920px] px-[40px]">
        ...
        <div className="mt-[28px] grid grid-cols-4 gap-[24px]">
          {courses.map((card, index) => (
            ...
            >
              <span className="course-badge course-badge--viewing">Trending</span>
              ...
              <div className="course-stats mt-[10px]">
                <span className="course-stat">
                  {card.rating ? card.rating.toFixed(1) : "4.7"} rating
                </span>
                <span className="course-stat">
                  {card.students ?? "10k"} viewers
                </span>
              </div>
              {card.lessons?.length ? (
